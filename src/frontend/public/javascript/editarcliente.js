document.addEventListener('DOMContentLoaded', async function () {
    ...
    form.addEventListener('submit', async function (event) {
        ...
        const clienteId = partes[partes.length - 2]
        const objeto = Object.fromEntries(new FormData(form))
        editarCliente(objeto,clienteId)
...
function liberar_nome_da_empresa_select(selecttipo, inputempresa, checkbox, pempresa) {
    let valorselect = selecttipo.value
    let nomeempresa = document.getElementById('empresaInput').value
    if(valorselect === 'empresa'){
        inputempresa.style.display = 'block'
        inputempresa.required
        checkbox.style.display = 'none'
        pempresa.style.display = 'none'
    }else if(valorselect === 'pessoa'){
        if(nomeempresa){
            inputempresa.style.display = 'block'
        }else{
...
async function editarCliente(formData,clienteId){
    try {
        const Dados = {
