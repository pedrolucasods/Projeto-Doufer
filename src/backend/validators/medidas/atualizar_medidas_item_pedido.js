function ValidatorAtualizarMedidaItemPedido(Dados,res){
    const campos = Object.keys(Dados)
    if(campos.length!=5){
        res.status(400).json({'erro':'Erro ao atualizar medida do Item Pedido!'})
        return false
        
    }
    if( !campos.includes('item_pedido_medida_id') ||
        !campos.includes('medida_id') ||
        !campos.includes('tipo_medida') ||
        !campos.includes('quantidade') ||
        !campos.includes('medidas') ){
        res.status(400).json({'erro':'Erro ao atualizar medida do Item Pedido!'})
        return false
    }
...
module.exports = {ValidatorAtualizarMedidaItemPedido}